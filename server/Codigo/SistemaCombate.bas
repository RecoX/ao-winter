...
Public Sub UserDañoNpc(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    ...
    With Npclist(NpcIndex)
        ...
        Call WriteChatOverHeadHit(UserIndex, "-" & daño, UserList(UserIndex).Char.CharIndex, 0, 0, 255)
...
Public Sub NpcDaño(ByVal NpcIndex As Integer, ByVal UserIndex As Integer)
    ...
    With UserList(UserIndex)
        ...
        Call WriteChatOverHeadHit(UserIndex, "-" & daño, Npclist(NpcIndex).Char.CharIndex, 0, 0, 255)
...
Public Function NpcAtacaUser(ByVal NpcIndex As Integer, ByVal UserIndex As Integer) As Boolean
    ...
    If NpcImpacto(NpcIndex, UserIndex) Then
        ...
        End With
    Else
        Call WriteChatOverHeadHit(UserIndex, "''Falla''", Npclist(NpcIndex).Char.CharIndex, 0, 0, 255)
...
On Error GoTo Errhandler
    ...
    If Npclist(NpcIndex).Numero = NPCReyCastle Then
        If UserList(UserIndex).GuildIndex = 0 Then
            Call WriteConsoleMsg(UserIndex, "No Tenes Clan. Para participar en la conquista de castillos necesita uno!", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        Else
            If Npclist(NpcIndex).Stats.MinHP = Npclist(NpcIndex).Stats.MaxHP Then
                Select Case UserList(UserIndex).Pos.map
                  Case WINTER
                        ...
                        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("El clan " & modGuilds.GuildName(UserList(UserIndex).GuildIndex) & " esta atacando al rey del castillo Winter", FontTypeNames.FONTTYPE_GUILD))
    ...
    Else
        Call WriteChatOverHeadHit(UserIndex, "''Fallas''", UserList(UserIndex).Char.CharIndex, 0, 0, 255)
...
On Error GoTo Errhandler
    ...
    With UserList(AtacanteIndex)
        ...
        Else
            ...
            Call WriteChatOverHeadHit(AtacanteIndex, "''Falla''", UserList(AtacanteIndex).Char.CharIndex, 0, 0, 255)
        End If
...
On Error GoTo Errhandler
    ...
    If UserList(AtacanteIndex).flags.Metamorfosis = 1 Then daño = daño + UserList(AtacanteIndex).Stats.DañoExtra 'Metamorfosis
    ...
    With UserList(AtacanteIndex)
        ...
        If UserList(AtacanteIndex).flags.Equitando = 1 Then
            Obj = ObjData(UserList(AtacanteIndex).Invent.MonturaObjIndex)
            daño = daño + RandomNumber(Obj.MinHIT, Obj.MaxHIT)
        End If
         
        If UserList(VictimaIndex).flags.Equitando = 1 Then
        ...
        If .flags.invisible = 1 Then
                ...
                .flags.invisible = 0
                Call WriteTimeInvi(AtacanteIndex, 0)
                Call UsUaRiOs.SetInvisible(AtacanteIndex, .Char.CharIndex, False)
                Call WriteConsoleMsg(AtacanteIndex, "¡Has vuelto a ser visible!", FontTypeNames.FONTTYPE_INFO)
        End If
    ...
    Call WriteChatOverHeadHit(AtacanteIndex, daño, UserList(AtacanteIndex).Char.CharIndex, 0, 0, 255)
...
'***************************************************
    ...
    If UserList(VictimIndex).flags.Meditando Then
        ...
        Call SendData(SendTarget.ToPCArea, VictimIndex, PrepareMessageCreateParticle(UserList(VictimIndex).Char.CharIndex, UserList(VictimIndex).Char.Particle, 1))
...
'***************************************************
    ...
    If Npclist(NpcIndex).Hostile = 0 Then
        ...
        ElseIf Npclist(NpcIndex).NPCtype = eNPCType.GuardiaReal Then
            ...
            If esArmada(attackerIndex) Then
                Call WriteConsoleMsg(attackerIndex, "No puedes atacar Guardias Temples siendo Caballero del Temple", FontTypeNames.FONTTYPE_INFO)
                PuedeAtacarNPC = False
                Exit Function
            End If
...
'***************************************************
    ...
    If ExpaDar > 0 Then
        ...
        Else
            Call AsignarExperiencia(UserIndex, ExpaDar)
        End If
    End If
End Sub
