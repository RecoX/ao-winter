    ...
    If InMapBounds(map, X, Y) Then
        With MapData(map, X, Y)
            ...
            If .TileExit.map > 0 And .TileExit.map <= NumMaps Then
                ...
                '¿Es mapa de newbies?
                If UCase$(MapInfo(.TileExit.map).Restringir) = eRestrict.restrict_newbie Then
                    ...
                    Else 'No es newbie
                        ...
                        End If
                    End If
                ElseIf UCase$(MapInfo(.TileExit.map).Restringir) = eRestrict.restrict_armada Then '¿Es mapa de Armadas?
                    ...
                    Else 'No es armada
                        ...
                        End If
                    End If
                ElseIf UCase$(MapInfo(.TileExit.map).Restringir) = eRestrict.restrict_caos Then '¿Es mapa de Caos?
                    '¿El usuario es Caos?
                    ...
                    Else 'No es caos
                        ...
                        End If
                    End If
                ElseIf UCase$(MapInfo(.TileExit.map).Restringir) = eRestrict.restrict_faccion Then '¿Es mapa de faccionarios?
       ...
       If UserList(TempCharIndex).flags.AdminInvisible = 0 Or UserList(UserIndex).flags.Privilegios And PlayerType.Dios Then
            ...
            If LenB(UserList(TempCharIndex).DescRM) = 0 And UserList(TempCharIndex).showName Then 'No tiene descRM y quiere que se vea su nombre.
               ...
               LevelMith = 7
                ...
                If UserList(TempCharIndex).Faccion.ArmadaReal = 1 Then
                    Stat = Stat & " <Caballero Temple> " & "<" & TituloReal(TempCharIndex) & ">"
                ElseIf UserList(TempCharIndex).Faccion.FuerzasCaos = 1 Then
                    Stat = Stat & " <Ejercito Oscuro> " & "<" & TituloCaos(TempCharIndex) & ">"
                End If
                                 
                If UserList(TempCharIndex).flags.Privilegios And PlayerType.RoyalCouncil Then
                    Stat = Stat & " [CONSEJO DEL TEMPLE]"
