...
'Evitamos sobrepasar los 255 (Byte) mediante
'la implementación de un nuevo Enum ;) [Maxtus]
...
On Error Resume Next
    ...
    If Not (packetID = ClientPacketID.LoginExistingChar _
      ...
      Or packetID = ClientPacketID.CuentaCL) Then
    ...
    Select Case packetID
        ...
        Case ClientPacketID.MsgGlobal               ';
            Call HandleMsgGlobal(UserIndex)
        ...
        Case ClientPacketID.PartyLeave              '/SALIRGRUPO
            Call HandlePartyLeave(UserIndex)
        
        Case ClientPacketID.PartyCreate             '/CREARGRUPO
            Call HandlePartyCreate(UserIndex)
        
        Case ClientPacketID.PartyJoin               '/GRUPO
        ...
        Case ClientPacketID.PartyOnline             '/ONLINEGRUPO
        ...
        Case ClientPacketID.PartyKick               '/ECHARGRUPO
            Call HandlePartyKick(UserIndex)
        
        Case ClientPacketID.PartySetLeader          '/GRUPOLIDER
            Call HandlePartySetLeader(UserIndex)
        
        Case ClientPacketID.PartyAcceptMember       '/ACCEPTGRUPO
        ...
        Case ClientPacketID.rank                    '/RANK
            Call HandleRank(UserIndex)
            
        Case ClientPacketID.topquince               '/TOP5
            Call HandleTopQuince(UserIndex)
        ...
        Case ClientPacketID.BanACC                  '/BANACC
            Call HandleBanAcc(UserIndex)
            
        Case ClientPacketID.UnbanACC                '/UNBANACC
            Call HandleUnbanAcc(UserIndex)
        ...
        Case ClientPacketID.SetIniVar               '/SETINIVAR LLAVE CLAVE VALOR
         ...
         Case ClientPacketID.CuentaCL
            Call HandleCuenta(UserIndex)
            
        Case ClientPacketID.WinterClientPacket
            Call HandleWinterClientPacket(UserIndex)
...
On Error GoTo Errhandler
    ...
    If Not FileExist(App.Path & "\Cuentas\" & accName & ".acc", vbNormal) Then
        Call WriteErrorMsg(UserIndex, "El nombre de la cuenta es inexistente.")
        Exit Sub
    End If
    
    If Not Password = GetVar(App.Path & "\Cuentas\" & accName & ".acc", "CUENTA", "PASSWORD") Then
    ...
    UserName = GetVar(App.Path & "\Cuentas\" & accName & ".acc", "PERSONAJES", "PJ" & PJ)
    ...
    If Not PersonajeExiste(UserName) Or UserName = "" Then
    ...
    End If
        ...
        If BANCheck(UserName) Then
            Call WriteErrorMsg(UserIndex, "Se te ha prohibido la entrada a Winter debido a tu mal comportamiento. Puedes consultar el reglamento y el sistema de soporte desde www.aowinter.com.ar")
        ElseIf Not VersionOK(version) Then
            Call WriteErrorMsg(UserIndex, "Esta version del juego es obsoleta, la version correcta es " & ULTIMAVERSION & ". Ejecute el launcher o descargelo desde la web www.winter-ao.com.ar")
        Else
...
On Error GoTo Errhandler
    ...
    MiCabeza = buffer.ReadInteger
    CuentaName = buffer.ReadASCIIString()
    
    With UserList(UserIndex).Stats
        .UserAtributos(eAtributos.Fuerza) = buffer.ReadByte()
        .UserAtributos(eAtributos.Agilidad) = buffer.ReadByte()
        .UserAtributos(eAtributos.Inteligencia) = buffer.ReadByte()
        .UserAtributos(eAtributos.Carisma) = buffer.ReadByte()
        .UserAtributos(eAtributos.Constitucion) = buffer.ReadByte()
    End With
        ...
        Else
            Call ConnectNewUser(UserIndex, UserName, race, gender, Class, MiCabeza, CuentaName)
        End If
...
On Error GoTo Errhandler
    With UserList(UserIndex)
        ...
        '    Call WriteConsoleMsg(UserIndex, "¿¡Como pretendes comunicarte contigo mismo!?", FontTypeNames.FONTTYPE_INFO)
        '    Exit Sub
        'End If
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If .flags.TimesWalk >= TiempoDeWalk Then
            ...
            If dummy < 5800 Then
                ...
                If Not .flags.CountSH = 0 Then
                    ...
                    If Not .flags.Anomalia = 3 Then
                    'Lorwik> Damos oportunidades a los usuarios que simplemente tengan lag
                        .flags.Anomalia = .flags.Anomalia + 1
                        Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor> Se ha encontrado una posible anomalia en " & .Name & " (Por posible uso de SH) [ADV:" & .flags.Anomalia & "]", FontTypeNames.FONTTYPE_SERVER))
                        Exit Sub
                    End If
        ...
        If .flags.Makro <> 0 Then
            Select Case .flags.Makro
                Case eMakro.Pescar, eMakro.PescarRed
                    Call WriteConsoleMsg(UserIndex, "Macro de Pesca asistido desactivado", FontTypeNames.FONTTYPE_VENENO)
                    
                Case eMakro.Talar
                    Call WriteConsoleMsg(UserIndex, "Macro de Tala asistido desactivado", FontTypeNames.FONTTYPE_VENENO)
        ...
        If .flags.Paralizado = 0 Then
            If .flags.Meditando Then
                ...
                Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateParticle(.Char.CharIndex, .Char.Particle, 1))
                .Char.Particle = 0
                .Char.loops = 0
            Else
                'Move user
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        Select Case Skill
            ...
            Case Ocultarse
                If .flags.Navegando = 1 Or .flags.Equitando = 1 Then
                    '[CDT 17-02-2004]
                    If Not .flags.UltimoMensaje = 3 Then
                        Call WriteConsoleMsg(UserIndex, "No podés ocultarte.", FontTypeNames.FONTTYPE_INFO)
        ...
        Select Case Skill
            Case eSkill.Proyectiles
                ...
                With .Invent
                    ...
                    Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(PROYECTIL, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y))
                End With
                ...
                If DummyINT > 0 Then
                    ...
                    If ObjData(DummyINT).OBJType = eOBJType.otArboles Then
                        If PuedeTalar(UserIndex) Then
                            .flags.Makro = eMakro.Talar
                            Call WriteConsoleMsg(UserIndex, "Macro de Tala asistido activado.", FontTypeNames.FONTTYPE_VENENO)
                        End If
                    Else
                ...
                If DummyINT > 0 Then
                    ...
                    If ObjData(DummyINT).OBJType = eOBJType.otYacimiento Then
                        If PuedeMinar(UserIndex) Then
                            .flags.Makro = eMakro.Minar
                            Call WriteConsoleMsg(UserIndex, "*MACRO DE MINERIA ASISTIDO ACTIVADO*", FontTypeNames.FONTTYPE_VENENO)
                        End If
                    Else
            ...
            Case FundirMetal    'UGLY!!! This is a constant, not a skill!!
                ...
                If PuedeLingotear(UserIndex) Then
                    .flags.Makro = eMakro.Lingotear
                    Call WriteConsoleMsg(UserIndex, "Macro de Fundición asistido activado.", FontTypeNames.FONTTYPE_VENENO)
                End If
            
            Case eSkill.Herreria
            
                If modCentinela.Centinela.RevisandoUserIndex = UserIndex Then
                    Call WriteConsoleMsg(UserIndex, "¡¡Contesta al centinela!!.", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
            
                If UserList(UserIndex).flags.Equitando Then
                    Call WriteConsoleMsg(UserIndex, "¡No puedes trabajar montado!.", FontTypeNames.FONTTYPE_INFO)
                    Exit Sub
                End If
...
On Error GoTo Errhandler
    With UserList(UserIndex)
        ...
        If modGuilds.CrearNuevoClan(UserIndex, desc, GuildName, site, codex, .FundandoGuildAlineacion, errorStr) Then
            Call QuitarObjetos(1163, 1, UserIndex)
        ...
        Else
            ...
            If .flags.Equitando = 1 Then
                If .Invent.MonturaSlot = Slot Then
                    Call WriteConsoleMsg(UserIndex, "No podés vender tu montura mientras lo estés usando.", FontTypeNames.FONTTYPE_TALK)
                    Exit Sub
                End If
            End If
...
'***************************************************
    With UserList(UserIndex)
        ...
        If .flags.Equitando = 1 Then
            Call WriteConsoleMsg(UserIndex, "No puedes meditar mientras equitas.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        ...
        If .flags.Meditando Then
            ...
            If .Stats.ELV < 13 Then
                .Char.Particle = ParticleIDs.ParticleMEDITARCHICO
            
            ElseIf .Stats.ELV < 25 Then
                .Char.Particle = ParticleIDs.ParticleMEDITARMEDIANO
            
            ElseIf .Stats.ELV < 35 Then
                .Char.Particle = ParticleIDs.ParticleMEDITARGRANDE
            
            ElseIf .Stats.ELV < 42 Then
                .Char.Particle = ParticleIDs.ParticleMEDITARXGRANDE
            
            Else
                .Char.Particle = ParticleIDs.PARTICLEMEDITARXXGRANDE
            End If
            
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessageCreateParticle(.Char.CharIndex, .Char.Particle, 0))
        Else
...
Dim Mensaje As String
    ...
    With UserList(UserIndex)
        ...
        'Si es un bug:
        ElseIf Tipo = 1 Then
            cant = GetVar(App.Path & "\REPORTES\BUG.INI", "BUGS", "CANTIDAD")
            Motivo = val(cant) + 1
            Nuevo = "Bug" & Motivo
            Mensaje = Date & " " & Time & " - " & UserList(UserIndex).Name & " Reporto el siguiente Bug: " & message & " - IP: " & UserList(UserIndex).ip

            Call WriteVar(App.Path & "\REPORTES\BUG.INI", "Bugs", "Cantidad", Motivo)
            Call WriteVar(App.Path & "\REPORTES\BUG.INI", "Reportes", Nuevo, Mensaje)
            
            Call WriteConsoleMsg(UserIndex, "El Bug ha sido reportado exitosamente!", FONTTYPE_GUILD)
            Call WriteConsoleMsg(SendTarget.ToAdmins, Mensaje, FONTTYPE_TALK)
            Exit Sub
        ElseIf Tipo = 2 Then
            cant = GetVar(App.Path & "\REPORTES\SUGERENCIAS.INI", "SUGERENCIAS", "CANTIDAD")
            Motivo = val(cant) + 1
            ...
            Call WriteVar(App.Path & "\REPORTES\SUGERENCIAS.INI", "SUGERENCIAS", "Cantidad", Motivo)
            Call WriteVar(App.Path & "\REPORTES\SUGERENCIAS.INI", "Reportes", Nuevo, Mensaje)
            
            Call WriteConsoleMsg(UserIndex, "La sugerencia ha sido reportado exitosamente!", FONTTYPE_GUILD)
            Call WriteConsoleMsg(SendTarget.ToAdmins, Mensaje, FONTTYPE_TALK)
            Exit Sub
        End If
...
On Error GoTo Errhandler
    With UserList(UserIndex)
        ...
        If Denuncias = False Then
                Call WriteConsoleMsg(UserIndex, "Las denuncias no estan activadas.", FontTypeNames.FONTTYPE_INFO)
                Call .incomingData.CopyBuffer(buffer)
                Exit Sub
        Else
            If .flags.Silenciado = 0 Then
                ...
                Call WriteConsoleMsg(UserIndex, "Denuncia enviada, espere pacientemente... (El abuso de denuncia puede llevar a sancion).", FontTypeNames.FONTTYPE_INFO)
            End If
        End If
    ...
    With UserList(UserIndex)
        ...
        If UserPuedeEjecutarComandos(UserIndex) Then
            ...
            Else
                ...
                Call WriteConsoleMsg(UserIndex, LCase(UserName) & " no pertenece a tu grupo.", FontTypeNames.FONTTYPE_INFO)
            End If
        End If
    ...
    With UserList(UserIndex)
        ...
        If UserPuedeEjecutarComandos(UserIndex) Then
            ...
            If tUser > 0 Then
                ...
                Else
                    Call WriteConsoleMsg(UserIndex, LCase(UserList(tUser).Name) & " no pertenece a tu grupo.", FontTypeNames.FONTTYPE_INFO)
                End If
    ...
    With UserList(UserIndex)
        ...
        If mdParty.UserPuedeEjecutarComandos(UserIndex) And bUserVivo Then
            ...
            Else
                ...
                'Don't allow users to spoof online GMs
                If (UserDarPrivilegioLevel(UserName) And rank) <= (.flags.Privilegios And rank) Then
                    Call WriteConsoleMsg(UserIndex, LCase(UserName) & " no ha solicitado ingresar a tu grupo.", FontTypeNames.FONTTYPE_PARTY)
                Else
                    Call WriteConsoleMsg(UserIndex, "No puedes incorporar a tu grupo a personajes de mayor jerarquía.", FontTypeNames.FONTTYPE_INFO)
                End If
            End If
        ...
        If Not .flags.Privilegios And PlayerType.User Then
            ...
            Else
                If UserList(tUser).flags.Silenciado = 0 Then
                    ...
                    Call WriteShowMessageBox(tUser, "ESTIMADO USUARIO, ud ha sido silenciado por los administradores.")
                ...
                Select Case opcion
                    ...
                    Case eEditOptions.eo_Experience
                        ...
                        If tUser <= 0 Then ' Offline
                            ...
                            Call WriteConsoleMsg(UserIndex, "Charfile Alterado: " & UserName, FontTypeNames.FONTTYPE_INFO)
                        Else ' Online
                            Call AsignarExperiencia(tUser, val(Arg1))
                        End If
                    ...
                    Case eEditOptions.eo_Body
                        If tUser <= 0 Then
                            ...
                            Call WriteConsoleMsg(UserIndex, "Charfile Alterado: " & UserName, FontTypeNames.FONTTYPE_INFO)
                        Else
                            Call ChangeUserChar(tUser, val(Arg1), UserList(tUser).Char.Head, UserList(tUser).Char.heading, UserList(tUser).Char.WeaponAnim, UserList(tUser).Char.ShieldAnim, UserList(tUser).Char.CascoAnim, UserList(tUser).Char.Aura)
                        End If
                    ...
                    Case eEditOptions.eo_Head
                        If tUser <= 0 Then
                            ...
                            Call WriteConsoleMsg(UserIndex, "Charfile Alterado: " & UserName, FontTypeNames.FONTTYPE_INFO)
                        Else
                            Call ChangeUserChar(tUser, UserList(tUser).Char.body, val(Arg1), UserList(tUser).Char.heading, UserList(tUser).Char.WeaponAnim, UserList(tUser).Char.ShieldAnim, UserList(tUser).Char.CascoAnim, UserList(tUser).Char.Aura)
                        End If
                    ...
                    Case eEditOptions.eo_Level
                        ...
                        If val(Arg1) >= 25 Then
                            ...
                            If GI > 0 Then
                                If modGuilds.GuildAlignment(GI) = "Ejercito Oscuro" Or modGuilds.GuildAlignment(GI) = "Caballeros del Temple" Then
            ...
            Else
                With UserList(tUser)
                    ...
                    If .flags.Muerto = 1 Then
                        ...
                        Call ChangeUserChar(tUser, .Char.body, .OrigChar.Head, .Char.heading, .Char.WeaponAnim, .Char.ShieldAnim, .Char.CascoAnim, UserList(tUser).Char.Aura)
...
''
' Handles the "BanAcc" message.
'
' @param    userIndex The index of the user sending the message.

Private Sub HandleBanAcc(ByVal UserIndex As Integer)
'***************************************************
'Author: Nicolas Matias Gonzalez (NIGO)
'Last Modification: 12/29/06
'
...
''
' Handles the "UnbanAcc" message.
'
' @param    userIndex The index of the user sending the message.

Private Sub HandleUnbanAcc(ByVal UserIndex As Integer)
'***************************************************
'Author: Nicolas Matias Gonzalez (NIGO)
'Last Modification: 12/29/06
'
    ...
    With UserList(UserIndex)
        ...
        If (Not .flags.Privilegios And PlayerType.RoleMaster) <> 0 And (.flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios)) <> 0 Then
            ...
            If Not FileExist(CharPath & UserName & ".chr", vbNormal) Then
                Call WriteConsoleMsg(UserIndex, "Charfile inexistente (no use +)", FontTypeNames.FONTTYPE_INFO)
            Else
                NameAccount = GetVar(CharPath & UserName & ".chr", "INIT", "Cuenta")
                
                If GetVar(App.Path & "\Cuentas\" & NameAccount & ".acc", "CUENTA", "Ban") = 1 Then
                    Call UnBanAccount(UserName)
                
                    'penas
                    cantPenas = val(GetVar(CharPath & UserName & ".chr", "PENAS", "Cant"))
                    Call WriteVar(CharPath & UserName & ".chr", "PENAS", "Cant", cantPenas + 1)
                    Call WriteVar(CharPath & UserName & ".chr", "PENAS", "P" & cantPenas + 1, LCase$(.Name) & ": UNBANACC. " & Date & " " & Time)
                
                    Call LogGM(.Name, "/UNBANACC a " & UserName)
                    Call WriteConsoleMsg(UserIndex, UserName & " unbanned.", FontTypeNames.FONTTYPE_INFO)
                Else
                    Call WriteConsoleMsg(UserIndex, "La cuenta de " & UserName & " no esta baneada. Es imposible unbanear", FontTypeNames.FONTTYPE_INFO)
                End If
            End If
        End If
...
Public Sub HandleWinterClientPacket(ByVal UserIndex As Integer)
'***************************************************
...
On Error Resume Next
    ...
    With UserList(UserIndex)
        ...
        Select Case packetID
            ...
            Case WinterClientPacketID.Quest                   '/QUEST
                Call HandleQuest(UserIndex)
                
            Case WinterClientPacketID.QuestAccept
                Call HandleQuestAccept(UserIndex)
            
            Case WinterClientPacketID.QuestListRequest
                Call HandleQuestListRequest(UserIndex)
            
            Case WinterClientPacketID.QuestDetailsRequest
                Call HandleQuestDetailsRequest(UserIndex)
            
            Case WinterClientPacketID.QuestAbandon
                Call HandleQuestAbandon(UserIndex)
 
            Case WinterClientPacketID.Noche
                Call HandleNoche(UserIndex)
            
            Case WinterClientPacketID.SwapObjects
                Call HandleSwapObjects(UserIndex)
            
            Case WinterClientPacketID.Canjes
                Call HandleCanje(UserIndex)
            
            Case WinterClientPacketID.Puntos
                Call HandlePuntos(UserIndex)
            
            Case WinterClientPacketID.DueloAutomatico
                Call HandleDueloAutomatico(UserIndex)
           
            Case WinterClientPacketID.CancelarDueloAutomatico
                Call HandleCancelarDueloAutomatico(UserIndex)
           
            Case WinterClientPacketID.ParticiparDueloAutomatico
                Call HandleParticiparDueloAutomatico(UserIndex)
            
            Case WinterClientPacketID.RequestPartyForm
                Call HandlePartyForm(UserIndex)
                
            Case WinterClientPacketID.invocar
                Call HandleInvocar(UserIndex)
            
            Case WinterClientPacketID.desdenuncia
                Call HandleDesDenuncia(UserIndex)
                
            Case WinterClientPacketID.HacerTorneo
                Call HandleHacerTorneo(UserIndex)
           
            Case WinterClientPacketID.TORNEO
                Call HandleTorneo(UserIndex)
               
            Case WinterClientPacketID.Cerrartorneo
                Call HandleCerrarTorneo(UserIndex)
                
            Case WinterClientPacketID.Transferencia
                Call handleTransferencia(UserIndex)
        End Select
    End With
End Sub
    ...
    With UserList(UserIndex)
        ...
        If (.flags.Privilegios And (PlayerType.Admin Or PlayerType.Dios Or PlayerType.SemiDios)) Then
            If LenB(message) <> 0 Then
                ...
                Call PostMensaje(UserList(UserIndex).Name & " > " & message)
            End If
        End If
...
On Error GoTo Errhandler
    With UserList(UserIndex)
        ...
        'Solo dioses, admins y RMS
        If .flags.Privilegios And (PlayerType.Dios Or PlayerType.Admin Or PlayerType.RoleMaster) Then
            Call SendData(SendTarget.ToRealYRMs, 0, PrepareMessageConsoleMsg("Caballeros del Temple> " & message, FontTypeNames.FONTTYPE_TALK))
        End If
...
''
' Handle the "Rank" message
'
' @param userIndex The index of the user sending the message

Public Sub HandleRank(ByVal UserIndex As Integer)
'***************************************************
...
'***************************************************
    With UserList(UserIndex)
        ...
        Call WriteConsoleMsg(UserIndex, "- El usuario con más nivel actualmente es: " & GetVar(App.Path & "\Rank.dat", "MaxNivel", "NivelName") & " Nivel: " & GetVar(App.Path & "\Rank.dat", "MaxNivel", "Nivel") & ".", FontTypeNames.FONTTYPE_PARTY)
        Call WriteConsoleMsg(UserIndex, "- El usuario con más oro actualmente es: " & GetVar(App.Path & "\Rank.dat", "MaxOro", "NombreOro") & " con la suma de: " & GetVar(App.Path & "\Rank.dat", "MaxOro", "Oro") & " monedas de oro.", FontTypeNames.FONTTYPE_PARTY)
        Call WriteConsoleMsg(UserIndex, "- El usuario con más muertes actualmente es: " & GetVar(App.Path & "\Rank.dat", "MaxDead", "NombreDead") & " con la suma de: " & GetVar(App.Path & "\Rank.dat", "MaxDead", "Muertes") & " muertes.", FontTypeNames.FONTTYPE_PARTY)
    End With
End Sub

''
' Handle the "TopQuince" message
'
' @param userIndex The index of the user sending the message

Public Sub HandleTopQuince(ByVal UserIndex As Integer)
'***************************************************
'Author: Manuel (Lorwik)
'Last Modification: 29/11/2011
'***************************************************
Dim i As Integer
    With UserList(UserIndex)
        ...
        For i = 1 To 5
            Call WriteConsoleMsg(UserIndex, "- En el puesto nº" & i & ": " & GetVar(App.Path & "\Rank.dat", "Top5", "NivelName" & i) & ", Nivel: " & GetVar(App.Path & "\Rank.dat", "Top5", "Nivel" & i) & ", con: " & GetVar(App.Path & "\Rank.dat", "Top5", "Muertes" & i) & " Muertes.", FontTypeNames.FONTTYPE_PARTY)
        Next i
    End With
End Sub
''
...
Public Sub WriteChangeMap(ByVal UserIndex As Integer, ByVal map As Integer, ByVal Zona As String)
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "ChangeMap" message to the given user's outgoing data buffer
...
Public Sub WriteCharacterChange(ByVal UserIndex As Integer, ByVal body As Integer, ByVal Head As Integer, ByVal heading As eHeading, _
                                ...
                                ByVal FX As Integer, ByVal FXLoops As Integer, ByVal helmet As Integer, ByVal Aura As Integer)
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "CharacterChange" message to the given user's outgoing data buffer
'***************************************************
On Error GoTo Errhandler
    Call UserList(UserIndex).outgoingData.WriteASCIIStringFixed(PrepareMessageCharacterChange(body, Head, heading, CharIndex, weapon, shield, FX, FXLoops, helmet, Aura))
Exit Sub
...
Public Sub WriteUpdateHit(ByVal UserIndex As Integer)
    ...
    End With
End Sub
Public Sub HandleCuenta(ByVal UserIndex As Integer)
Dim Command, CuentaPass, CuentaEmail As String
...
With UserList(UserIndex)
    ...
    Select Case Command
        Case "LoginAcc"
            CuentaName = .incomingData.ReadASCIIString
            CuentaPass = .incomingData.ReadASCIIString
           
            Call ConectarCuenta(UserIndex, CuentaName, CuentaPass)
            Exit Sub
        ...
        Dim resp As String
            ...
            preg = .incomingData.ReadASCIIString
            resp = .incomingData.ReadASCIIString
            
            Call CrearCuenta(UserIndex, CuentaName, CuentaPass, CuentaEmail, preg, resp)
            Exit Sub
           
        Case "DltPJAcc"
            Dim PJClickeado As Byte
            Dim ACPassword As String
            
            CuentaName = .incomingData.ReadASCIIString
            ACPassword = .incomingData.ReadASCIIString
            PJClickeado = .incomingData.ReadByte
            
            Call BorrarPersonaje(UserIndex, CuentaName, PJClickeado, ACPassword)
            Exit Sub
            
        Case "Cmbdpss"
            Dim PsswdAnte As String
            Dim PasswdNew As String
            
            CuentaName = .incomingData.ReadASCIIString
            PsswdAnte = .incomingData.ReadASCIIString
            PasswdNew = .incomingData.ReadASCIIString
            
            Call CambiarPassword(UserIndex, CuentaName, PsswdAnte, PasswdNew)
            
        Case "RcprAcc"
            Dim NewPassword As String
            Dim Preguntita As String
            Dim Respuesta As String
            
            CuentaName = .incomingData.ReadASCIIString
            NewPassword = .incomingData.ReadASCIIString
            Preguntita = .incomingData.ReadASCIIString
            Respuesta = .incomingData.ReadASCIIString
            
            Call RecuperarCuenta(UserIndex, CuentaName, NewPassword, Preguntita, Respuesta)
            Exit Sub
    End Select
End With
End Sub
 
Public Sub EnviarCuenta(ByVal UserIndex As Integer, ByVal PJName As String, ByVal CantPjs As Byte, ByVal Total As Byte, ByVal estado As Byte)
With UserList(UserIndex).outgoingData
    .WriteByte (ServerPacketID.CuentaSV)
    .WriteASCIIString ("LoginOK")
    .WriteByte (estado)
   
    .WriteByte (CantPjs)
    
    If Not CantPjs = 0 Then
        .WriteASCIIString (PJName)
        .WriteByte (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "INIT", "Clase"))
        .WriteByte (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "STATS", "ELV"))
        .WriteLong (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "REP", "Promedio"))
        .WriteInteger (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "INIT", "Body"))
        .WriteInteger (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "INIT", "Head"))
        .WriteByte (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "INIT", "Casco"))
        .WriteByte (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "INIT", "Escudo"))
        .WriteByte (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "INIT", "Arma"))
        .WriteByte (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "FLAGS", "Ban"))
        .WriteByte (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "FLAGS", "Muerto"))
        .WriteByte (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "INIT", "Raza"))
        .WriteByte (GetVar(App.Path & "\Charfile\" & PJName & ".chr", "INIT", "Logged"))
    End If
...
If .flags.Privilegios And PlayerType.User Then Exit Sub
    ...
    ElseIf Anocheceria = 3 Then
        For i = 1 To LastUser
          Call writeNoche(i, 3)
        Next i
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("La noche cae sobre las tierras de Winter. Las criaturas de la noche salen a cazar. [Noche]", FontTypeNames.FONTTYPE_GUILD))
...
Private Sub HandleMsgGlobal(ByVal UserIndex As Integer)
If UserList(UserIndex).incomingData.length < 3 Then
    Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
    Exit Sub
End If
...
With UserList(UserIndex)
    ...
    If LenB(Chat) <> 0 Then
        ...
        Call Statistics.ParseChat(Chat)
        If GlobalAC Then
            'Si el usuario esta silenciado, no habla.
            If .flags.Silenciado = 1 Then
                Call WriteConsoleMsg(UserIndex, "¡Estas silenciado!.", FontTypeNames.FONTTYPE_TALK)
                Call .incomingData.CopyBuffer(buffer)
                Exit Sub
            End If
            'Prevencion de Hacks de colores-
            Dim LoopC As Integer
            For LoopC = 1 To Len(Chat)
                If LCase$(mid$(Chat, LoopC, 1)) = "~" Then
                    Call WriteConsoleMsg(UserIndex, "El texto contiene caracteres invalidos.", FontTypeNames.FONTTYPE_TALK)
                    Call .incomingData.CopyBuffer(buffer)
                    Exit Sub
                End If
            Next LoopC
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(.Name & "> " & Chat, FontTypeNames.FONTTYPE_GLOBAL))
                Call PostMensaje(.Name & "> " & Chat)
                Call LogGlobal(.Name & "> " & Chat)
        Else
            Call WriteConsoleMsg(UserIndex, "El chat global se encuentra desactivado.", FontTypeNames.FONTTYPE_TALK)
        End If
    End If
...
With UserList(UserIndex)
    ...
    GlobalAC = True
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("El sistema de mensaje global ha sido activado. Se ruega no abusar de el. Para hablar su mensaje deberá contener el prefijo "";""", FontTypeNames.FONTTYPE_VENENO))
    Call PrepareMessagePlayWave(43, .Pos.X, .Pos.Y)
End With
End Sub
...
With UserList(UserIndex)
    ...
    GlobalAC = False
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("El sistema de mensaje global ha sido desactivado.", FontTypeNames.FONTTYPE_VENENO))
    Call PrepareMessagePlayWave(45, .Pos.X, .Pos.Y)
End With
End Sub

Private Sub HandleCanje(ByVal UserIndex As Integer)
Dim NumCanje As Byte
With UserList(UserIndex)
    Call .incomingData.ReadByte
    NumCanje = .incomingData.ReadByte()
    Call Canjeos(UserIndex, NumCanje)
End With
End Sub

Private Sub HandlePuntos(ByVal UserIndex As Integer)
...
On Error GoTo Errhandler
    With UserList(UserIndex)
        ...
        If (Not .flags.Privilegios And PlayerType.RoleMaster) <> 0 And (Not .flags.Privilegios And PlayerType.User) <> 0 Then
             ...
             If LenB(UserName) = 0 Then
                 Call WriteConsoleMsg(UserIndex, "Comando Incorrecto. Utilice /Puntos NICKNAME@PUNTOS.", FontTypeNames.FONTTYPE_INFO)
             Else
                 tUser = NameIndex(UserName)
             End If
             ...
             If FileExist(CharPath & UserName & ".chr", vbNormal) Then
                 UserList(tUser).Stats.Puntos = UserList(tUser).Stats.Puntos + Puntos
                 Call WriteConsoleMsg(tUser, "¡Has recibido " & Puntos & " Puntos!", FontTypeNames.FONTTYPE_INFO)
                 Call WriteConsoleMsg(UserIndex, "Has entregado " & Puntos & " puntos a " & UserList(tUser).Name, FontTypeNames.FONTTYPE_INFO)
             Else
...
Public Sub WriteCanjesInit(ByVal UserIndex As Integer)
'***************************************************
'Author: Juan Martín Sotuyo Dodero (Maraxus)
'Last Modification: 05/17/06
'Writes the "CommerceInit" message to the given user's outgoing data buffer
...
On Error GoTo Errhandler
With UserList(UserIndex)
    Call .outgoingData.WriteByte(ServerPacketID.CanjesInit)
    .outgoingData.WriteLong (UserList(UserIndex).Stats.Puntos)
End With
Exit Sub
...
Public Function PrepareMessageCreateParticle(ByVal CharIndex As Integer, ByVal Particle As Integer, ByVal SeBorra As Byte) As String
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.ParticleCreate)
        Call .WriteInteger(CharIndex)
        Call .WriteInteger(Particle)
        Call .WriteByte(SeBorra)
...
On Error GoTo Errhandler
With UserList(UserIndex)
    Call .outgoingData.WriteByte(ServerPacketID.TimeInvi)
...
On Error GoTo Errhandler
With UserList(UserIndex)
    Call .outgoingData.WriteByte(ServerPacketID.TimeParal)
    .outgoingData.WriteByte (Time)
End With
Exit Sub
...
Public Sub HandleDueloAutomatico(UserIndex As Integer)
With UserList(UserIndex)
    Call .incomingData.ReadByte
     Dim rondas As Integer
    rondas = .incomingData.ReadInteger()
    If Not UserList(UserIndex).flags.Privilegios < SemiDios Then
        If (rondas > 0 And rondas < 6) Then Call Torneos_Inicia(UserIndex, rondas)
    Else
        Call WriteConsoleMsg(UserIndex, "Solo los administradores del juego pueden usar este comando", FontTypeNames.FONTTYPE_INFO)
    End If
End With
End Sub
           
Public Sub HandleCancelarDueloAutomatico(UserIndex As Integer)
    Call UserList(UserIndex).incomingData.ReadByte
    If Not UserList(UserIndex).flags.Privilegios < SemiDios Then
        Call Rondas_Cancela
    Else
...
Public Sub HandleParticiparDueloAutomatico(UserIndex As Integer)
    Call UserList(UserIndex).incomingData.ReadByte
...
Dim TiCoordX1 As Byte
Dim TiCoordY1 As Byte
Dim TiCoordX2 As Byte
Dim TiCoordY2 As Byte
Dim TiCoordX3 As Byte
Dim TiCoordY3 As Byte
Dim InvTitanX As Byte
Dim InvTitanY As Byte

    With UserList(UserIndex)
        'Remove packet ID
        Call .incomingData.ReadByte
        
        '******Comprobamos que el Titan no este invocado******
        If Titan = True Then
            Call WriteConsoleMsg(UserIndex, "¡El Titan ya esta invocado!", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        '******************************************************
        '*********Comprobamos en que mapa se encuentra el usuario para asignar los datos********
        'Titan de Fuego
        If UserList(UserIndex).Pos.map = 165 Then
            'Mapa y Coordeanadas de los usuarios.
            MapaTitan = 165
            TiCoordX1 = 53
            TiCoordY1 = 65
            TiCoordX2 = 57
            ...
            TiCoordX3 = 61
            TiCoordY3 = 65
            'Coordenada del Spawn del Titan
            InvTitanX = 57
            InvTitanY = 65
            'Numero del NPC Titan
            TitanIndex = 20
        'Titan de Tierra
        ElseIf UserList(UserIndex).Pos.map = 169 Then
            'Mapa y Coordeanadas de los usuarios.
            MapaTitan = 169
            TiCoordX1 = 27
            TiCoordY1 = 28
            ...
            TitanIndex = 29
        'Titan de Agua
        ElseIf UserList(UserIndex).Pos.map = 167 Then
            'Mapa y Coordeanadas de los usuarios.
            MapaTitan = 167
            TiCoordX1 = 50
            TiCoordY1 = 34
        ...
        '*******Ya que tenemos todos los datos, ejecutamos el codigo.*********
        If MapData(MapaTitan, TiCoordX1, TiCoordY1).UserIndex > 0 And MapData(MapaTitan, TiCoordX2, TiCoordY2).UserIndex > 0 And MapData(MapaTitan, TiCoordX3, TiCoordY3).UserIndex > 0 Then
     
            Invocadores(1) = MapData(165, TiCoordX1, TiCoordY1).UserIndex
            Invocadores(2) = MapData(165, TiCoordX2, TiCoordY2).UserIndex
            Invocadores(3) = MapData(165, TiCoordX3, TiCoordY3).UserIndex
            ...
            tmpWP.X = InvTitanX
            tmpWP.Y = InvTitanY
            Call SpawnNpc(TitanIndex, tmpWP, True, False)
        
        Else
            Call WriteConsoleMsg(UserIndex, "¡Debeis de colocaros en los puntos de invocacion!", FontTypeNames.FONTTYPE_INFO)
        End If
        '*********************************************************************
    End With
End Sub

Private Sub HandleDesDenuncia(ByVal UserIndex As Integer)
'***************************************************
'Author: Lorwik
'Last Modification: 12/08/2012
'
'***************************************************
Dim Activador As Byte

    With UserList(UserIndex)
        'Remove packet ID
        Call .incomingData.ReadByte
        Activador = .incomingData.ReadByte()
        
        If .flags.Privilegios And (PlayerType.User) Then Exit Sub
        
        If Activador = 1 Then
            If Denuncias = True Then
                Call WriteConsoleMsg(UserIndex, "Las denuncias ya estan activadas", FontTypeNames.FONTTYPE_INFO)
            Else
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Las denuncias han sido activadas.", FontTypeNames.FONTTYPE_INFO))
                Denuncias = True
            End If
        Else
            If Denuncias = False Then
                Call WriteConsoleMsg(UserIndex, "Las denuncias ya estan activadas", FontTypeNames.FONTTYPE_INFO)
            Else
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Las denuncias han sido desactivadas.", FontTypeNames.FONTTYPE_INFO))
                Denuncias = False
            End If
        End If
    End With
...
Private Sub HandleHacerTorneo(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        Call .incomingData.ReadByte
        MinLevel = .incomingData.ReadByte
        MaxLevel = .incomingData.ReadByte
        Cupos = .incomingData.ReadByte
        AutoSum = .incomingData.ReadByte
        If AutoSum = 1 Then
        mapa = .incomingData.ReadInteger
        X = .incomingData.ReadByte
        Y = .incomingData.ReadByte
        ...
        If Hay_Torneo = False Then
            ...
            Dim data As String
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("[Torneo organizado por " & UserList(UserIndex).Name & "]", FontTypeNames.FONTTYPE_WARNING))
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Nivel máximo: " & MaxLevel & "", FontTypeNames.FONTTYPE_GM))
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Nivel Mínimo: " & MinLevel & "", FontTypeNames.FONTTYPE_GM))
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Cupo máximo: " & Cupos & "", FontTypeNames.FONTTYPE_GM))
           ' Dim i As Integer
            'For i = 1 To 8
           ...
           ' Data = Data & "."
           ' Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Clases válidas: " & Data & "", FontTypeNames.FONTTYPE_INFO))
           ' Data = ""
           ' For i = 1 To 4
            ...
            'Data = Left$(Data, Len(Data) - 1)
            'Data = Data & "."
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Alineación válidas: Todas ", FontTypeNames.FONTTYPE_GM))
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Escribe ""/TORNEO"" para participar.", FontTypeNames.FONTTYPE_GM))
    ...
    End With
End Sub
Public Sub HandleTorneo(ByVal UserIndex As Integer)
With UserList(UserIndex)
Call .incomingData.ReadByte
 
 
            If Hay_Torneo = False Then
            Call WriteConsoleMsg(UserIndex, "No hay ningún torneo disponible.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
            End If
           
            If UserList(UserIndex).Stats.ELV < MinLevel Then
                Call WriteConsoleMsg(UserIndex, "Tu nivel es: " & UserList(UserIndex).Stats.ELV & ".El requerido es: " & MinLevel, FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
           
            If UserList(UserIndex).Stats.ELV > MaxLevel Then
                Call WriteConsoleMsg(UserIndex, "Tu nivel es: " & UserList(UserIndex).Stats.ELV & ".El máximo es: " & MaxLevel, FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
            If Torneo_Inscriptos >= Cupos Then
                Call WriteConsoleMsg(UserIndex, "El cupo ya ha sido alcanzado.", FontTypeNames.FONTTYPE_INFO)
                Exit Sub
            End If
...
'                Call WriteConsoleMsg(UserIndex, "Tu clase no es válida en este torneo.", FontTypeNames.FONTTYPE_INFO)
 '               Exit Sub
 '               End If
     '       Next
            ...
            If Not UserList(UserIndex).flags.EnTorneo = True Then
                Call WriteConsoleMsg(UserIndex, "Estás en la lista de espera del torneo. Estás en el puesto nº " & Torneo_Inscriptos + 1, FontTypeNames.FONTTYPE_INFO)
                .flags.EnTorneo = True
               
                Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("/TORNEO [" & UserList(UserIndex).Name & "]", FontTypeNames.FONTTYPE_INFOBOLD))
                Torneo_Inscriptos = Torneo_Inscriptos + 1
                If Torneo_Inscriptos = Cupos Then
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Cupo alcanzado.", FontTypeNames.FONTTYPE_EXP))
                End If
                If AutoSum = 1 Then
...
'                Call Torneo.Quitar(UserList(Userindex).Name)
                Call WriteConsoleMsg(UserIndex, "Ya estás en la lista de espera del torneo.", FontTypeNames.FONTTYPE_INFO)
'                Torneo_Inscriptos = Torneo_Inscriptos - 1
...
End With
End Sub
Public Sub HandleCerrarTorneo(ByVal UserIndex As Integer)
    With UserList(UserIndex)
        Call .incomingData.ReadByte
        If Hay_Torneo = True Then
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Las incripciones al torneo fueron cerradas", FontTypeNames.FONTTYPE_EXP))
            Hay_Torneo = False
            Dim i As Long
            For i = 1 To LastUser
                UserList(i).flags.EnTorneo = True
            Next i
        Else
            Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("No hay ningun torneo disponible", FontTypeNames.FONTTYPE_INFO))
        End If
    End With
End Sub

Public Sub handleTransferencia(ByVal UserIndex As Integer)
'***************************
'Autor: Lorwik
'***************************
Dim TName As String
...
Dim tUser As Integer
 ...
 With UserList(UserIndex)
        ...
        TName = .incomingData.ReadASCIIString
        TCantidad = .incomingData.ReadLong
        
        tUser = NameIndex(TName)
        
        '1º - Compruebo si el usuario esta online.
        If tUser <= 0 Then
            Call WriteConsoleMsg(UserIndex, "Usuario offline.", FontTypeNames.FONTTYPE_INFO)
        '2º - Compruebo si la cantidad de oro que quiere dar es valida
        ElseIf .Stats.GLD < TCantidad Then
            Call WriteConsoleMsg(UserIndex, "No puedes dar más oro del que tienes", FontTypeNames.FONTTYPE_CITIZEN)
        '3º - Compruebo que el usuario destinatario no es el mismo que envia el oro
        ElseIf tUser = UserIndex Then
            Call WriteConsoleMsg(UserIndex, "No puedes hacer una transferencia a ti mismo.", FontTypeNames.FONTTYPE_INFO)
        '4º - Realizo la transferencia
        Else
            .Stats.GLD = .Stats.GLD - TCantidad
            UserList(tUser).Stats.GLD = UserList(tUser).Stats.GLD + TCantidad
           
            Call WriteConsoleMsg(UserIndex, "Has transferido " & TCantidad & " monedas de oro a " & UserList(tUser).Name & ".", FONTTYPE_CENTINELA)
            Call WriteConsoleMsg(tUser, "El usuario " & .Name & " te hizo una transferencia de " & TCantidad & " monedas de oro.", FONTTYPE_CENTINELA)
        End If
End With
End Sub
